...
CREATE TABLE IF NOT EXISTS `app_user` (
  ...
  -- 认证相关
  `login_type` TINYINT(4) NOT NULL COMMENT '登录方式: 1-微信, 2-手机号验证码',
  `phone_number` VARCHAR(20) DEFAULT NULL COMMENT '手机号(手机号登录时必填)',
  `wechat_open_id` VARCHAR(64) DEFAULT NULL COMMENT '微信OpenID(微信登录时必填)',
  -- `password_hash` 已移除，采用短信验证码登录
  ...
  UNIQUE KEY `uk_wechat` (`wechat_open_id`),
